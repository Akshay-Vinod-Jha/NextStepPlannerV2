...
export const AdminCard = ({ destination }) => {
  ...
  const handleViewBookings = () => {
    console.log("Navigate to view bookings for destination:", destination);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await fetch(
        getApiUrl(`/destinations/deletedestn/${destination._id}`),
        {
          method: "DELETE",
        }
      );
  ...
  const sampleDestination = destination || {
    ...
    altitude: "3,583m",
    description:
      "Experience the spiritual journey to one of the most sacred temples in India. This trek offers breathtaking views of the Himalayas and a chance to connect with nature.",
    images: [
      {
        ...
        <div className="relative overflow-hidden">
          <img
            ...
            className={`w-full h-64 object-cover transition-transform duration-300 hover:scale-110 ${
              sampleDestination.status === "unavailable"
                ? "grayscale-[30%]"
                : ""
            }`}
          />
          ...
          <div className="absolute bottom-4 left-4">
            <div
              className={`px-3 py-1 rounded-full flex items-center space-x-1 font-semibold text-xs uppercase backdrop-blur-sm ${
          ...
          <div className="absolute top-4 right-4">
            <div className="bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full flex items-center space-x-1">
              ...
              <span className="text-sm font-bold text-gray-900">
                {sampleDestination.price.toLocaleString()}
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl p-6 max-w-md w-full mx-4 shadow-2xl">
            <div className="text-center">
              ...
              <p className="text-gray-600 mb-6">
                Are you sure you want to delete "{sampleDestination.name}"? This
